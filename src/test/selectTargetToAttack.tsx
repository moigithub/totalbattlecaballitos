...
describe('selectTargetToAttack', () => {
  ...
  const enemyStacks: FightStack[] = [
    createFightStack('7', 'Walls', 'fortification', '', 10000, 30000, 0, 0, 700, {
      vsMeleePercent: 100,
      vsRangedPercent: 100,
  ...
  test('should select Ent VI as first target for fearsomeManticoraV (strongest available)', () => {
    ...
    const target = selectTargetToAttack(attacker, enemyStacks, new Set())
  ...
  test('should select battleGriffinV as target for Unicorn Rider V (biggest threat)', () => {
    ...
    const target = selectTargetToAttack(attacker, playerStacks, new Set())
  ...
  test('should select fearsomeManticoraV as target for Ent VI (strongest available)', () => {
    ...
    const target = selectTargetToAttack(modifiedAttacker, playerStacks, new Set())
    expect(target?.unit.name).toBe('fearsomeManticoraV')
  })
  ...
  test('should select Elf archer I as target for CatapultE4 (strongest available)', () => {
    ...
    const target = selectTargetToAttack(attacker, modifiedEnemyStacks, new Set())
  ...
  test('should return null when no valid targets exist', () => {
    ...
    const target = selectTargetToAttack(attacker, emptyEnemies, new Set())
...
describe('Additional selectTargetToAttack tests', () => {
  ...
  const enemyStacks: FightStack[] = [
    createFightStack('e1', 'Walls', 'fortification', '', 10000, 30000, 0, 0, 135000, {
      vsMeleePercent: 100,
      vsRangedPercent: 100,
      vsMountedPercent: 100
    }),
...
describe('FirePhoenix vsMelee bonus target selection', () => {
  test('should select Ent VI over Bear V for firePhoenixI when total damage with bonuses is higher against Ent', () => {
    ...
    // Verify selection
    const target = selectTargetToAttack(firePhoenix, [bear, ent], new Set())
    ...
    expect(firePhoenixDamage).toBeLowerThan(bearHealth)
    expect(firePhoenixDamage).toBeLowerThan(entHealth)

    expect(firePhoenixMaxDamage).toBeGreaterThan(bearHealth)
    expect(firePhoenixMaxDamage).toBeLowerThan(entHealth)
  })
})
