...
export const Card = ({
  ...
  otherStrengthInfo.push(
    ...
    >
      ...
      Total Dmg: {allBonusesPercent > 0 && <>({allBonusesPercent}%)</>} {totalDamage.toFixed(0)}
    </span>
  )
  ...
  return (
    ...
    >
      ...
      <div className='card-section-2'>
        <p className='stack-name text-md md:text-2xl lg:text-3xl font-bold overflow-ellipsis'>
          {stack.unit.name}
        </p>
        <div className='stack-attack-info text-xs'>
          <span className='text-yellow-500'>{stack.unit.group}</span>
          {stack.unit.subGroup && <span className='text-fuchsia-500'>:{stack.unit.subGroup}</span>}
          {stack.unit.category && <span className='text-orange-500'>:{stack.unit.category}</span>}
          <span className='text-red-600'>{' vs '}</span>
          <span className='text-green-600'>{whoCanIAttack(stack.unit).join(',')}</span>
        </div>
        <div className='stack-leadership text-cyan-300 text-xs'>
          {stack.unit.clasification === 'army' && <span>Leadership {stack.leadership}</span>}
          {stack.unit.clasification === 'monster' && <span>Dominance {stack.dominance}</span>}
          {stack.unit.clasification === 'merc' && <span>Authority {stack.authority}</span>}
        </div>

        <div className='stack-basic-info text-sm'>
          Unit{' '}
          <span className=' text-red-600'>Hp {unitHealth.toLocaleString().replace(/,/g, '_')}</span>{' '}
          <span className='  text-teal-600'>
            Str {unitStrength.toLocaleString().replace(/,/g, '_')}
          </span>
        </div>
        <p className='stack-health-strength flex flex-wrap text-sm'>
          Stack{' '}
        ...
        <div className='bonus flex mt-2'>
          <div className='stack-strBonus mr-4'>
      ...
      <div className='card-section-3'>
        ...
        <p className='stack-units my-2 font-bold text-lg sm:text-2xl md:text-3xl flex justify-center items-center bg-green-800 text-gray-300 py-1 sm:py-2 md:py-4'>
          {stack.unitsAmount}
        </p>

        <div className='stack-action w-full flex justify-between items-center '>
          <button
            className='text-3xl bg-gray-500  cursor-pointer  inline-flex items-center justify-center border border-gray-300 rounded-md h-[40px] w-[60px] focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none'
            onClick={() => {
              addUnits(stack.id!, 1)
            }}
          >
